...
function SavingCaseModal({ isOpen, progress = 0, totalDocuments = 0, currentDocument = 0 }) {
    if (!isOpen) return null;

    const progressPercentage = Math.min(Math.max(progress, 0), 100);

    // Determinar mensaje según el progreso
    let statusMessage = 'Creando caso...';
    if (currentDocument > 0 && totalDocuments > 0) {
        statusMessage = `Guardando documentos ${currentDocument}/${totalDocuments}`;
    } else if (progressPercentage > 0 && progressPercentage < 100) {
        statusMessage = 'Guardando caso...';
    }
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm animate-fadeIn">
            <div className="bg-stone-50 rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 animate-slideUp">
                <div className="flex flex-col items-center gap-">
                    {/* Imagen de carga */}
                    <img src={iconLoading} alt="Guardando caso..." className="w-52 h-52 object-contain" />
...
export default SavingCaseModal;
