...
export default function EditUserModal({
  ...
  const handleSave = async () => {
    ...
    // Actualizar colegio si cambió
    const currentColegioId = user.colegios_info && user.colegios_info.length > 0 ? user.colegios_info[0].id : null;
    if (selectedColegio !== currentColegioId) {
      await onCambiarColegio(user.id, selectedColegio);
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto transform transition-all">
        <div className="sticky top-0 bg-white/80 backdrop-blur-md px-5 py-4 md:px-8 md:py-6 flex items-center justify-between z-10 border-b border-gray-100">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Modifica la información del usuario existente.</p>
          </div>
          <button
            onClick={onClose}
