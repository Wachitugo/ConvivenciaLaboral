            ...
            <div className="fixed right-0 top-0 h-full z-[70] flex items-center justify-end pointer-events-none" style={{ fontFamily: "'Poppins', sans-serif" }}>
                <div className="w-[430px] h-full shadow-[0_0_40px_rgba(0,0,0,0.5)] bg-[#0A3866]/95 backdrop-blur-3xl border-l border-[#1A71B8]/30 flex flex-col animate-slide-in overflow-hidden pointer-events-auto">
                    ...
                    <div className="px-6 py-5 border-b border-white/10 flex justify-between items-center">
                        <div>
                            <h2 className="text-lg font-bold text-white">Asociar Entrevista</h2>
                            <p className="text-xs text-white/60 mt-0.5">
                                Selecciona una entrevista para vincular al colaborador
                            </p>
                        </div>
                        <button
                            onClick={onClose}
                    ...
                    <div className="flex-1 overflow-y-auto p-6 custom-scrollbar space-y-5">
                        ...
                        <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                            <p className="text-xs font-bold text-white/50 uppercase tracking-widest mb-1">Vinculando a</p>
                            <p className="text-sm font-bold text-white">{student?.nombres} {student?.apellidos}</p>
                            {student?.curso && (
                                <span className="inline-block mt-2 text-xs font-mono bg-white/10 px-2 py-0.5 rounded border border-white/10 text-white/60">
                                    {student.curso}
                                </span>
                            )}
                        </div>
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-white/50 uppercase tracking-widest">Buscar Entrevista</label>
                            <div className="relative">
                                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-white/40" size={18} />
                                <input
                                    type="text"
                                    placeholder="Buscar por colaborador o entrevistador..."
                                    value={searchTerm}
                                    onChange={(e) => setSearchTerm(e.target.value)}
                                    className="w-full pl-11 pr-4 py-3 rounded-xl border border-white/20 focus:border-[#34B6D8] focus:ring-4 focus:ring-[#34B6D8]/10 outline-none bg-white/5 text-sm font-medium text-white transition-all placeholder:text-white/30"
                                />
                            </div>
                        </div>
                        ...
                        {/* Interviews List */}
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-white/50 uppercase tracking-widest">Entrevistas Disponibles</label>
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`p-3.5 rounded-xl border cursor-pointer transition-all flex items-center gap-3 ${isSelected
                                                ? 'border-[#1A71B8]/60 bg-[#1A71B8]/20 ring-1 ring-[#1A71B8]/40'
                                                : isAssociated
                                                    ? 'border-white/10 bg-white/5 opacity-50 cursor-not-allowed'
                                                    : 'border-white/10 bg-white/5 hover:border-white/20 hover:bg-white/10'
                                                }`}
                                        >
                                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${isSelected ? 'bg-[#1A71B8] text-white' : isAssociated ? 'bg-white/10 text-white/30' : 'bg-white/10 text-white/50'
                                                }`}>
                                                <FileText size={20} />
                                            </div>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-center mb-0.5">
                                                    <span className={`text-sm font-medium block truncate ${isSelected ? 'text-white' : 'text-white/80'}`}>
                                                        {i.studentName}
                                                    </span>
                                            ...
                                            {isAssociated ? (
                                                <span className="flex-shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-white/10 text-white/40">
                                                    Asociado
                                                </span>
                                            ) : isSelected && (
                                                <div className="w-5 h-5 rounded-full bg-[#1A71B8] flex items-center justify-center">
                    ...
                    <div className="p-6 border-t border-white/10">
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                className={`flex-1 px-4 py-2.5 text-white rounded-xl text-sm font-bold transition-colors shadow-md flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed border border-white/10 ${selectedInterviewIsAlreadyAssociated
